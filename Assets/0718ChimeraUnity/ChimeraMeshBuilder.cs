...
namespace Chimera
{
    ...
    public static class ChimeraMeshBuilder
    {
        ...
        public static Mesh Build(ZooidParams zp, Zone zone, OrganSettings cfg)
        {
            ...
            if (cfg.mouths && (isHead || rnd(131) > 0.45f))
            {
            ...
            if (cfg.headBuds && !isHead && rnd(161) > 0.55f)
            {
            ...
            if (cfg.limbs && !isHead && rnd(191) > 0.5f)
            {
            ...
            if (!isHead && amount > 0.01f)
            {
                ...
                else if (zone == Zone.Siphosome)     // 觸手 + 囊泡 + 突觸
                {
